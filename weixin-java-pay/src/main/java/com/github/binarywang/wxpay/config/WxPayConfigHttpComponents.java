...
@Getter
@Setter
public class WxPayConfigHttpComponents extends WxPayConfig {
   ...
   */
  private HttpClientBuilderCustomizerHC httpClientBuilderCustomizer;
  private HttpClientBuilderCustomizerHC apiV3HttpClientBuilderCustomizer;
  ...
  public CloseableHttpClient initApiV3HttpClient() throws WxPayException {
    ...
    try {
      ...
      //初始化V3接口正向代理设置
      HttpProxyUtilsHC.initHttpProxy(wxPayV3HttpClientBuilder, wxPayHttpProxy);
  ...
  private void configureProxy(HttpClientBuilder httpClientBuilder) {
    if (StringUtils.isNotBlank(this.getHttpProxyHost()) && this.getHttpProxyPort() > 0) {
      ...
      provider.setCredentials(new AuthScope(this.getHttpProxyHost(), this.getHttpProxyPort()),
        new UsernamePasswordCredentials(this.getHttpProxyUsername(),
          this.getHttpProxyPassword() == null ? null : this.getHttpProxyPassword().toCharArray()));
